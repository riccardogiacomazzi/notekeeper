const testContent = [
  {
    title: "La Luna Plena",
    content: `The quick brown fox jumps over the lazy dog.
It runs through the forest and leaps over streams.
Even the smallest creatures watch in awe.`,
    image:
      "https://d2w9rnfcy7mm78.cloudfront.net/24486474/original_ad0432f1022117279e023fbab6fbfa15.jpg?1698901359?bc=0",
    author: "Alice",
    tags: ["nature", "animal", "night"],
    comments: [
      { content: "Beautiful imagery!", author: "Sfilippy" },
      { content: "Reminds me of a dream.", author: "Aldo" },
      { content: "So vivid!", author: "Moro" },
    ],
  },
  {
    title: "Starlit Paths",
    content: `Stars shine brightest in the darkest nights.
Look up and let your thoughts wander among constellations.
Every twinkle tells a story of distant worlds.`,
    image:
      "https://d2w9rnfcy7mm78.cloudfront.net/15110892/original_d7eb6e3432faf402408c7ab79bd1b51c.jpg?1644501180?bc=0",
    author: "Bob",
    tags: ["astronomy", "inspiration", "dreams"],
    comments: [
      { content: "This feels cosmic.", author: "Diana" },
      { content: "Makes me want to stargaze!", author: "Eve" },
      { content: "Very peaceful.", author: "Charlie" },
    ],
  },
  {
    title: "First Steps",
    content: `A journey of a thousand miles begins with a single step.
Take courage and move forward, even when the path is unknown.
Each step brings new lessons and discoveries.`,
    image:
      "https://d2w9rnfcy7mm78.cloudfront.net/39002829/original_9a00e07390106b35f2f47c31109ab9d5.png?1756066786?bc=0",
    author: "Charlie",
    tags: ["motivation", "life", "growth"],
    comments: [
      { content: "This inspires me!", author: "Alice" },
      { content: "Courage is key.", author: "Sfilippy" },
      { content: "Needed this today.", author: "Bob" },
    ],
  },
  {
    title: "Raindrop Symphony",
    content: `Raindrops on leaves make a soothing melody.
The scent of wet earth fills the air.
Sit quietly and let nature calm your mind.`,
    image:
      "https://d2w9rnfcy7mm78.cloudfront.net/23768396/original_2732c8fdb445ea01f6fca3d76568bedd.jpg?1695350399?bc=0",
    author: "Diana",
    tags: ["nature", "rain", "calm"],
    comments: [
      { content: "So soothing!", author: "Eve" },
      { content: "I can almost hear it.", author: "Moro" },
      { content: "Love rainy days.", author: "Bob" },
    ],
  },
  {
    title:
      "Coffee & Companions nella jungla a sllevare pesi per scolpire i muscoli con le liane e le radiciCoffee & Companions nella jungla a sllevare pesi per scolpire i muscoli con le liane e le radici, lugnezza corta ripeto la frase; ",
    content: `Coffee tastes better when shared with friends.
Laughter echoes across the room.
Memories are brewed alongside every cup.`,
    image:
      "https://d2w9rnfcy7mm78.cloudfront.net/17334228/original_9e3770caead5ab2402b7883f54add3b1.jpg?1658718544?bc=0",
    author: "Eve",
    tags: ["food", "friends", "cozy"],
    comments: [
      { content: "Now I want coffee!", author: "Charlie" },
      { content: "So cozy and warm.", author: "Alice" },
      { content: "Perfect vibe.", author: "Diana" },
    ],
  },
  {
    title: "no image text ",
    content: `L'img non ce
sono un poeta maledetto
Memories are brewed while I listen to dnb.`,
    image: "",
    author: "Jesu",
    tags: ["tech", "berlin"],
    comments: [
      { content: "Now I want coffee!", author: "Charlie" },
      { content: "So cozy and warm.", author: "Alice" },
      { content: "Perfect vibe.", author: "Diana" },
    ],
  },
];

export default testContent;
